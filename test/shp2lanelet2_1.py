...
#车道边界线
EdgeLink_file = shapefile.Reader("/home/fmw/map/shp2lanelet2/demo_shp/demo_shp_20211217/HD_EdgeLink.shp") 

#车道边界线节点
EdgeNode_file = shapefile.Reader("/home/fmw/map/shp2lanelet2/demo_shp/demo_shp_20211217/HD_EdgeNode.shp") 

#车道中心线
LaneLink_file = shapefile.Reader("/home/fmw/map/shp2lanelet2/demo_shp/demo_shp_20211217/HD_LaneLink.shp") 

#转换后的osm文件路径
lanelet2_path = os.path.join("/home/fmw/map/shp2lanelet2/lanelet2_map.osm")
...
# 获取车道左/右边界Id
def get_EdgeId(lane, flag):
    id = -1
    id = lane[flag]
    # print(left_id)
    # if id not in EdgeLink['id']:
    #     print("不存在id=%s的车道边界线" %id)
    #     return -1
    # else:
    #     return id
    return id
...
def read_EdgeLink(file_path):
    ...
    while i < len(border):
        ...
        # Link = {'id':border[i].record[1], 'SE_NodeId':border[i].record[4], 'EE_NodeId':border[i].record[5], 'Edge_type':border[i].record[5], 'Edge_cross':border[i].record[5]} # 目前demo数据无type及跨越方向信息
        i = i + 1
        print(edge)
...
def write_lane(lane, projector, flag):
    print("-----------------------------Write_Lane---------------------------")
    leftedge_id = get_EdgeId(lane, flag) # 获取左边界线编号
    # print(leftedge_id)
    if leftedge_id == -1:
        print("车道线左边界ID读取错误")
        return -1
